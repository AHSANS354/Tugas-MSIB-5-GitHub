class Gempa {
  constructor(lokasi, skala) {
    this.lokasi = lokasi;
    this.skala = skala;
  }
  dampak() {
    const dampak = "";
    if (this.skala <= 2) {
      dampak = "Tidak terasa";
    } else if (this.skala > 2 && this.skala <= 4) {
      dampak = "Bangunan retak-retak";
    } else if (this.skala > 4 && this.skala <= 6) {
      dampak = "Bangunan roboh";
    } else if (this.skala > 6) {
      dampak = "Bangunan roboh dan berpotensi tsunami";
    }
  }
  cetak() {
    document.write("Lokasi Gempa : <br>", this.lokasi);
    document.write("<br> Skala Gempa : <br>", this.skala);
    document.write("<br> Dampak Gempa : <br>", dampak);
    document.write("<hr>");
  }
}
const g1 = new Gempa("39km dari kota ternate", 6.0);
const g2 = new Gempa("16km dari kabupaten donggala", 6.0);
const g3 = new Gempa("10km dari adassil, maroko", 6.8);
const g4 = new Gempa("33km dari jalisco, meksiko", 5.8);
const g5 = new Gempa("12km dari burney, california", 5.0);

const hasil = [g1, g2, g3, g4, g5];

function printGempa() {
  for (s = 0; s < hasil.length; s++) {
    const print = hasil[s].dampak();
  }
  document.getElementById("print").innerHTML = print;
}
